...
import logging
logger = logging.getLogger('bCanvasWIdget')
...
class bCanvasWidget(QtWidgets.QMainWindow):
    def __init__(self, filePath, parent=None, isNew=True):
        ...
        parent: bCanvasApp
        """
        print('bCanvasWidget.__init__() filePath:', filePath, 'isNew:', isNew)
        ...
        if self.isNew: # and self.getOptions()['motor']['useMotor']:
            ...
            # we loaded
            #print('bCanvasWidget is setting initial (artificial) motor position')
            xDisplay = -186541.5
            yDisplay = -186541.5
    ...
    def grabImage(self):
        ...
        logger.info('=== bCanvasWidget.grabImage()')
        ...
        logger.info(f'  options umWidth:{umWidth} umHeight:{umHeight}')
    ...
    def toggleMotor(self):
        ...
        print(f'bCanvasWIdget.toggleMotor() isVisible:{isVisible}')
        if isVisible:
            self.motorToolbarWidget.hide()
...
class myQGraphicsView(QtWidgets.QGraphicsView):
    ...
    def appendScopeFile(self, newScopeFile):
        ...
        if stackMax is None:
            ...
            print(f'  myQGraphicsView.appendScopeFile() NO MAX, using header imageStackWidth {imageStackWidth} imageStackHeight {imageStackHeight}')
        else:
    ...
    def keyPressEvent(self, event):
        ...
        if event.key() in [QtCore.Qt.Key_Plus, QtCore.Qt.Key_Equal]:
            ...
            print('myQGraphicsView.keyPressEvent() key calling centerOnCrosshair()')
